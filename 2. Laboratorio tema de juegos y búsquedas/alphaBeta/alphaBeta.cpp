...
public:
	...
	void generarArbol (Node *actual, int jugador) {
		if ( actual->height == maxHeight or actual->estadoActual.jugadasRestantes == 0 ) {
			...
			for ( int i = 0; i < actual->estadoActual.jugadasRestantes; ++i ){
				...
				if ( actual->height % 2) { /// padre es MINimizador
					if (actual->beta > actual->nodes[i]->estadoActual.utilidad) {
						actual->beta= actual->nodes[i]->estadoActual.utilidad;
						actual->camino = actual->nodes[i];
					}
				} else { /// padre es MAXimizador
					// if ( actual->beta <= actual->alpha)
					// 	break;
					if (actual->alpha < actual->nodes[i]->estadoActual.utilidad) {
						actual->alpha = actual->nodes[i]->estadoActual.utilidad;
						actual->camino = actual->nodes[i];
					}
				}
				if ( actual->beta <= actual->alpha)
		...
		for ( int i = 0; i < 3; ++i ) {
			for ( int j = 0; j < 3; ++j ) {
				if ( actual->estadoActual.lista[i][j] == -1 ) {
					...
					actual->nodes[pos]->alpha = actual->alpha;
					actual->nodes[pos]->beta = actual->beta;
	...
	void checkArbol (strJuego &tablero) {
		...
		/// elegir la jugada
		Node *siguiente_jugada = root->camino;
...
void pantallaGanador(int ganador) {
	...
	char jugar[10] = {'P','l','a','y',' ','A','g','a','i','n'};
	/// jugador
	if (ganador == 1) { 
		colorGreen;
